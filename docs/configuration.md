...
- `export` does not write what Braze says about it. For the
  directory-backed kinds that means no file is written at all. The
  single-file Custom Attribute registry is the exception in form only:
  an excluded entry already in `registry.yaml` is carried through
  untouched rather than refreshed or dropped — the file has to be
  rewritten as a whole, and deleting the entry would be a statement
  about it. (Excluded names are still *read* from the Braze list
  response there; `export` has to know which names the workspace
  returned before it can tell an out-of-band entry from a registry-only
  one.) See [registry-mode.md](registry-mode.md#what-export-writes).
