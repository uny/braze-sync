...
/// file tree don't expect `apply` to create attributes — and, since
/// `export` keeps entries it did not fetch, that the file is not purely
/// a dump of one workspace.
...
/// Returns `Ok(None)` when the file does not exist (valid for a fresh
...
/// Attributes are sorted by name so the file is deterministic across runs.
