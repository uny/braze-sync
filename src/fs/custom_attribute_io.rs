...
pub fn save_registry(path: &Path, registry: &CustomAttributeRegistry) -> Result<()> {
    ...
    let yaml = serde_norway::to_string(&normalized).map_err(|source| Error::YamlParse {
        path: path.to_path_buf(),
        source,
    })?;

    let mut out = String::from("# Generated by braze-sync.\n#\n");
    out.push_str("# This is a REGISTRY of Custom Attributes that exist in Braze.\n");
    out.push_str("# braze-sync cannot create new Custom Attributes — Braze creates them\n");
    out.push_str("# implicitly when /users/track receives data containing them.\n");
    out.push_str("# braze-sync can only mark them as deprecated.\n");
    out.push_str(&yaml);
